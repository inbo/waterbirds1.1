#' Calculate dropping model for given non-breeding bird species
#'
#' This function quantifies the allochthonous nitrogen and phosphorus input
#' by waterbirds based on nutrient concentrations in the daily
#' excrement productions (excretion or dropping model).
...
give_dropping_model <- function(
  ...
  result <- bird_abundance |>
    ...
    group_by(.data$location, .data$year, .data$month, .data$species) |>
    mutate(
